...
function GroomModal({ onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('Failed to log grooming activity');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="✨ Log Grooming">
      <div className="form-group">
        <label>Notes (optional)</label>
        <textarea
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          placeholder="Bath, brushing, nail trim, etc..."
          maxLength={5000}
        />
      </div>
